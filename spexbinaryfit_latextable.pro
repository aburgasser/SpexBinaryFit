...
; -----------------------------------------------------------------------------------------------------
indb, outfile, index=index, title=title, newobs=newobs, standalone=standalone
...
refs = [db.discovery_reference(index),db.optical_reference(index),db.nir_reference(index)]
if (keyword_set(newobs) eq 0) then refs = [refs,db.data_reference(index)]
...
if (keyword_set(newobs)) then $
 printf, unit, '\begin{deluxetable}{lllllccccl}' else $
...
for i=0,n_elements(index)-1 do begin
 if (i gt 0 and db.name(index(i)) eq db.name(index((i-1)>0)) and db.designation(index(i)) eq db.designation(index((i-1)>0))) then $
  line=' & & & & & & & ' else begin
  line = db.name(index(i))+' & '+db.designation(index(i))+' & '
 ...
 endelse
 if (keyword_set(newobs)) then line=line+spexbinaryfit_dateconvert(db.observation_date(index(i)))+' & '+db.resolution(index(i))+' & '
; references
 if (db.discovery_reference(index(i)) ne '') then line=line+strtrim(string(where(refs(s(u)) eq db.discovery_reference(index(i)))+1),2) else line=line+'\nodata'
 line=line+'; '
 if (db.optical_reference(index(i)) eq '' and db.nir_reference(index(i)) eq '') then line=line+'\nodata' else begin
  if (db.optical_reference(index(i)) ne '') then line=line+strtrim(string(where(refs(s(u)) eq db.optical_reference(index(i)))+1),2)
  ...
  if (db.nir_reference(index(i)) ne '' and db.nir_reference(index(i)) ne db.optical_reference(index(i))) then line=line+strtrim(string(where(refs(s(u)) eq db.nir_reference(index(i)))+1),2)
 endelse
 if (keyword_set(newobs) eq 0) then line=line+'; '+strtrim(string(where(refs(s(u)) eq db.data_reference(index(i)))+1),2)
 printf, unit, line+' \\'
; print, line
endfor
printf, unit, '\enddata
...
if (keyword_set(newobs)) then $
 printf, unit, '\tablenotetext{b}{First citation is the discovery reference; next citation(s) are classification references (optical and near-infrared).}' else $
 printf, unit, '\tablenotetext{b}{First citation is the discovery reference; next citation(s) are classification references (optical and near-infrared); final citation is the data reference.}
line='\tablerefs{'
for i=0,n_elements(u)-1 do line=line+'('+strtrim(string(i+1),2)+') \citet{'+refs(s(u(i)))+'}; '
